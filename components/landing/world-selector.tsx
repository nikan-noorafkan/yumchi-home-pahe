...
export function WorldSelector({ onSelect, isVisible }: WorldSelectorProps) {
  ...
  return (
    ...
    >
      ...
      >
        <h2 className="text-2xl font-bold text-foreground">Choose your world</h2>
        <p className="text-muted-foreground mt-1 text-sm">
          Where does your food journey begin?
        </p>
      </motion.div>
      ...
      >
        <div className="relative z-10 flex flex-col items-center gap-4 p-8 lg:p-12">
          ...
          <motion.div
            className="absolute bottom-6 left-6 text-secondary/30"
            animate={{ y: [5, -5, 5], rotate: [0, -10, 0] }}
          ...
          <div className="text-center">
            ...
            <p className="text-muted-foreground text-sm leading-relaxed max-w-xs">
              Your gamified cooking universe. Level up your skills, discover smart recipes,
              and master the kitchen.
            </p>
          </div>

          {/* Feature pills */}
          <div className="flex flex-wrap justify-center gap-2 mt-2">
            {["Gamified XP", "Smart Recipes", "Meal Plans"].map((f) => (
              <span
                key={f}
          ...
          <motion.div
            ...
            style={{ color: "#e75e3c" }}
            animate={{ x: hoveredWorld === "cook" ? [0, 4, 0] : 0 }}
            transition={{ duration: 0.8, repeat: hoveredWorld === "cook" ? Infinity : 0 }}
          >
            <Sparkles className="w-4 h-4" />
            Enter Kitchen
            <motion.span
              animate={{ x: hoveredWorld === "cook" ? [0, 6, 0] : 0 }}
              transition={{ duration: 0.8, repeat: hoveredWorld === "cook" ? Infinity : 0 }}
            >
              &rarr;
            </motion.span>
      ...
      <div className="flex items-center justify-center lg:mx-8 shrink-0">
        <div className="relative flex items-center justify-center w-14 h-14 rounded-full bg-card border-2 border-border shadow-lg">
          <span className="text-muted-foreground font-bold text-sm">or</span>
        </div>
      </div>
      ...
      >
        <div className="relative z-10 flex flex-col items-center gap-4 p-8 lg:p-12">
          {/* Floating icons */}
          <motion.div
            className="absolute top-4 left-6 text-accent/20"
          ...
          <motion.div
            className="absolute bottom-6 right-6 text-secondary/25"
            animate={{ y: [4, -6, 4] }}
          ...
          <motion.div
            ...
            animate={{
              boxShadow:
                hoveredWorld === "maps"
                  ? "0 8px 32px rgba(91,194,170,0.4)"
                  : "0 4px 16px rgba(91,194,170,0.2)",
            }}
          >
          ...
          <div className="text-center">
            ...
            <p className="text-muted-foreground text-sm leading-relaxed max-w-xs">
              Discover signature dishes and perfect restaurants. Your mood-based food
              adventure awaits.
            </p>
          </div>
          ...
          {/* Feature pills */}
          <div className="flex flex-wrap justify-center gap-2 mt-2">
            {["Dish Radar", "Mood Match", "Food City"].map((f) => (
              <span
                key={f}
                className="text-xs font-medium px-3 py-1 rounded-full"
                style={{
          ...
          <motion.div
            className="mt-4 flex items-center gap-2 text-sm font-semibold"
            style={{ color: "#5bc2aa" }}
            animate={{ x: hoveredWorld === "maps" ? [0, 4, 0] : 0 }}
            transition={{ duration: 0.8, repeat: hoveredWorld === "maps" ? Infinity : 0 }}
          >
            <Sparkles className="w-4 h-4" />
            Explore Maps
            <motion.span
              animate={{ x: hoveredWorld === "maps" ? [0, 6, 0] : 0 }}
              transition={{ duration: 0.8, repeat: hoveredWorld === "maps" ? Infinity : 0 }}
            >
              &rarr;
            </motion.span>
